...
/** Where a visitor of this page belongs, or null when they belong here. */
function profileRedirect(user: {
	username?: string;
	onboardingCompleted?: boolean;
}): string | null {
	if (user.onboardingCompleted) return "/chat";
	if (!user.username) return "/setup-profile";
	return null;
}
...
export default function OnboardingPage() {
	...
	// Redirecting from an effect, not mid-render: the early returns this replaces
	// sat above `useCallback` below, so the hook order changed between renders.
	const redirectTo = isLoading || !user ? null : profileRedirect(user);
	useEffect(() => {
